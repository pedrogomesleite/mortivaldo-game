...
def handleInterface(events, scene_manager, layers):
    ...
    for obj in layers[1]:
        ...
        if obj_class == "Slider":
            if MOUSEBUTTONDOWN in [event.type for event in events] and obj.sprites["handle"]["rect"].collidepoint(
                    mouse_pos):
                obj.active = True
            ...
            if obj.active:
                ...
                new_value = 100 * (mouse_pos[0] - obj.sprites["fill"]["rect"].left) / obj.sprites["fill"]["rect"].size[
                    0]
